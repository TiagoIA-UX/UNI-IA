# =============================================================
# DESTINO DO SCRIPT : C:\Users\omago\Downloads\APLICAR_WEBSOCKET.ps1
# EXECUTE ASSIM no PowerShell (qualquer pasta):
#   & "$env:USERPROFILE\Downloads\APLICAR_WEBSOCKET.ps1"
# =============================================================
...
Write-Host ''
Write-Host '== UNI IA — Aplicando WebSocket Bybit ==' -ForegroundColor Cyan
Write-Host ''

# ── 1. Verificar que o projeto existe ────────────────────────
if (-not (Test-Path $PROJECT)) {
    Write-Host 'ERRO: Pasta E:\UNI.IA nao encontrada.' -ForegroundColor Red
    exit 1
}

# ── 2. Criar live-signal-feed.tsx ────────────────────────────
Write-Host '[1/5] Criando live-signal-feed.tsx...' -ForegroundColor Yellow
$tsxPath = "$PROJECT\zairyx-blog\app\plataforma\live-signal-feed.tsx"
$tsxContent = @'
'use client';
...
# ── 3. Adicionar CSS ─────────────────────────────────────────
Write-Host '[2/5] Adicionando CSS ao plataforma.module.css...' -ForegroundColor Yellow
$cssPath = "$PROJECT\zairyx-blog\app\plataforma\plataforma.module.css"
$cssContent = @'
.liveFeedContainer {
  width: 100%;
  margin-top: 2rem;
...
.biasLONG    { background: #d5f5e3; color: #1e8449; border: 1px solid #58d68d; }
.biasSHORT   { background: #fadbd8; color: #922b21; border: 1px solid #e74c3c; }
.biasNEUTRAL { background: #f4f4f4; color: #555555; border: 1px solid #cccccc; }
...
/* Demais células */
.liveFeedVol { font-family: monospace; color: #555555; }
.liveFeedAge { font-size: 0.75rem; color: #888888; white-space: nowrap; }
...
Write-Host '[3/5] Verificando .env.local...' -ForegroundColor Yellow
$envPath = "$PROJECT\.env.local"
$envLine = 'NEXT_PUBLIC_BYBIT_SIGNAL_ASSETS=BTCUSDT,ETHUSDT,SOLUSDT'
$existing = Get-Content $envPath -Raw -ErrorAction SilentlyContinue
if ($existing -notmatch 'NEXT_PUBLIC_BYBIT_SIGNAL_ASSETS') {
    Add-Content -Path $envPath -Value "`n$envLine" -Encoding UTF8
    Write-Host '    OK -> variavel adicionada ao .env.local' -ForegroundColor Green
} else {
    Write-Host '    SKIP -> variavel ja existe' -ForegroundColor Yellow
}

# ── 5. Build ─────────────────────────────────────────────────
Write-Host '[4/5] Executando build Next.js...' -ForegroundColor Yellow
npm --prefix "$PROJECT\zairyx-blog" run build
...
# ── 6. Git commit e push ─────────────────────────────────────
Write-Host '[5/5] Commit e push...' -ForegroundColor Yellow
Set-Location $PROJECT
git add zairyx-blog/app/plataforma/live-signal-feed.tsx `
        zairyx-blog/app/plataforma/plataforma.module.css `
        .env.local
git commit -m "feat: replace HTTP polling with Bybit WebSocket real-time feed"
git push origin main

Write-Host ''
Write-Host '============================================' -ForegroundColor Green
Write-Host '  CONCLUIDO! Feed WebSocket ativo.' -ForegroundColor Green
Write-Host '  Rode: npm --prefix zairyx-blog run dev' -ForegroundColor Green
